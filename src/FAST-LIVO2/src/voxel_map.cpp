...
void VoxelMapManager::clearMemOutOfMap(const int& x_max,const int& x_min,const int& y_max,const int& y_min,const int& z_max,const int& z_min )
{
  ...
  std::cout<<YELLOW<<"before reserve voxel_map_.bucket_count() = "<<voxel_map_.bucket_count()<<RESET<<"\n";
  voxel_map_.reserve(voxel_map_.size());
  std::cout<<RED<<"[DEBUG]: Delete "<<delete_voxel_cout<<" root voxels"<<RESET<<"\n";
  std::cout<<GREEN<<"after  reserve voxel_map_.bucket_count() = "<<voxel_map_.bucket_count()<<RESET<<"\n";
